...
def suggest_delta(rows: list[list[int]]) -> float | None:
    """The best leveling angle, or None without a suggestion."""
    deltas = suggest_deltas(rows)
...
def test_horizontal_tilt_positive():
    ...
    delta = suggest_delta(edge_rows(3.0))
...
def test_horizontal_tilt_negative():
    ...
    delta = suggest_delta(edge_rows(-2.4))
...
def test_already_level():
    ...
    delta = suggest_delta(edge_rows(0.0))
    assert delta is not None
...
def test_vertical_structure():
    ...
    delta = suggest_delta(edge_rows(2.0, vertical=True))
...
def test_precision_across_the_window(tilt):
    ...
    delta = suggest_delta(edge_rows(tilt))
    assert delta is not None
...
def test_single_edge_yields_one_candidate():
    ...
    assert len(suggest_deltas(edge_rows(3.0))) == 1
