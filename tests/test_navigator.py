...
from grawji.navigator import _POINT_ROTATIONS, _RECT_ROTATIONS
...
def test_full_rect_stays_full(rot):
    ...
    assert _RECT_ROTATIONS[rot](0.0, 0.0, 1.0, 1.0) == pytest.approx(
        (0.0, 0.0, 1.0, 1.0)
    )
...
def test_point_is_inverse_of_rect(rot, a, b):
    ...
    px, py = _POINT_ROTATIONS[rot](a, b)
    back = _RECT_ROTATIONS[rot](px, py, 0.0, 0.0)
...
def test_quarter_turns_swap_width_and_height(rot):
    ...
    _, _, w, h = _RECT_ROTATIONS[rot](0.1, 0.2, 0.3, 0.4)
    if rot == 180:
...
def test_rect_90_maps_left_strip_to_bottom():
    ...
    assert _RECT_ROTATIONS[90](0.0, 0.0, 0.5, 1.0) == pytest.approx(
        (0.0, 0.5, 1.0, 0.5)
    )
